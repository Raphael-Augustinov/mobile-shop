...
if (isset($_GET['order'])) { 
  $id = $_GET['order'];
  $results = mysqli_query($con, "SELECT * FROM `order-items` WHERE order_id=$id"); 
}
?>
</head>
...
<table class="table">
	...
	<?php while ($row = mysqli_fetch_array($results)) { ?>
		<tr>
			<th style="font-weight:normal;" scope="row"><?php echo $_GET['order']; ?></td>
            <td><?php echo $row['item_id']; ?></td>
            <td><?php $order_item=get_product_info($con,$row['item_id']);
                    echo $order_item['item_name'];  ?></td>
            <td>
                <?php
                    echo $row['currency'].' '.$row['item_price'];
                ?>
            </td>   
            <td>
    ...
    <!-- start #footer -->
    <footer id="footer" class="bg-dark text-white py-5" style="position:absolute;bottom:0;width:100%;height:12rem">
        <div class="container">
            <div class="row">
                <div class="col-lg-4 col-12">
                    ...
                    <p class="font-rale font-size-14 text-white-50">
                        ...
                        consectetur adipiscing elit. In ante metus, 
                        efficitur nec sem eu, lacinia imperdiet dolor. 
                        Curabitur nec velit sagittis, convallis nibh nec, dictum elit. 
                    </p>
                </div>
